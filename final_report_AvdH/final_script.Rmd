...
*NOTE: It is important to run the chunks one by one and to read the instructions in this markdown file.*
...
Code in the block below shows the pre-processing steps that were done in the original manuscript. I follow the same steps but do no run the text rank algorithm and do not apply the dictionary filter and exclusion of methodological terms. 

Steps that are applied: 
- to lower case
- udpipe ("english"): (tokenization, POS, stemming, nouns and adjectives)
...
# Steps in original manuscript up until just before the methodological filter is applied
...
Apply more pre-processing steps before conducting cluster analysis.  
...
I also save a date frame with only the filter column and keeping only the unique terms. This filter can than be easily applied on the data sets with the pre-trained GloVe and Word2Vec word vectors to extract the word vectors for the terms in the final filter.  
...
Therefore it is important that the following file, `pretrained_w2v_filtered.csv` is in the folder `final_report_AvdH`. To create the .csv file you will need to run the following Python script: `create_w2v_embedding.py.` 
...
Note that the existing pre-trained word vectors from the Word2Vec dataset are extracted in a Python file and the code below only loads the resulted dataframe, cleans the dataframe and saves the dataframe as an .RData object.

Therefore it is important that the following file, `pretrained_w2v_filtered_bigrams.csv` is in the folder `final_report_AvdH`. To create the .csv file you will need to run the following Python script: `create_w2v_emb_bigrams.py`. 
...
In the block of code below the silhouette plot that is discussed in the final report is created. However there were some issues with Quick Transfer warnings and the use of other techniques (not discussed in the final report) also took a really long time or could not be performed due to the large size of the data. Therefore it is advised to run the block below line by line. For reproducing the silhouette plot in the paper run the line below the comment in capitals. 

Examples on how to use the factoextra package are retrieved from the following page: 
https://statsandr.com/blog/clustering-analysis-k-means-and-hierarchical-clustering-by-hand-and-in-r/#silhouette-method

Note that there is also a package, clValid, that can help choosing an appropriate clustering algorithm next to the optimal number of k. For more information on this package see the following pages:
...
*DO NOT RUN THIS BLOCK OF CODE BELOW IN ONE GO AS IT WILL TAKE A REALLY LONG TIME* -> The lines of code that need to be run to reproduce the silhouette plot presented in the report are announced by a comment in capital letters. 
```{r}
# Elbow method
set.seed(88)
elbow_plot_glove <- fviz_nbclust(glove_embedding, kmeans, iter.max = 40, nstart = 25, method = "wss", k.max = 150) +
  labs(subtitle = "Elbow method") # add subtitle
# NOTE:this took a really long time and gave Quick Transfer messages. The elbow plot that is used in the report is the one created with the wssplot function.  
...
To easily present the cluster assignment in the report, the clusters are visualized separately through the means of t-SNE. More information on visualizing high dimensional data using t-SNE can be found in the following paper: https://www.jmlr.org/papers/volume9/vandermaaten08a/vandermaaten08a.pdf. 

How the create the plot from the t-SNE results object has been retrieved and adapted from the following tutorial: https://www.r-bloggers.com/2019/03/sentiment-analysis-word-embedding-and-topic-modeling-on-venom-reviews/.  
```{r}
...
## Check which cluster contains the months in the results of the application of kmeans with GloVe word vectors and k = 125
Since it was found that when applying DBSCAN, nine months of the year were put together in one cluster, it was checked to which cluster these terms belonged in the application of the k-means algorithm (k = 125) that was conducted earlier on. Next to this, a cluster analysis was conducted within this specific cluster in which these months could be found. 
```{r}
# find out to which cluster one of the months, august, is assigned
words_with_cluster125[words_with_cluster125$word == "august",]
cluster125_110 <- subset(words_with_cluster125, subset=kmeans125 == 110)

## Cluster analysis within cluster 
# create a matrix that only contains the terms and their associated word vectors that belong to one cluster (cluster 110)
...
# try to get rid of remaining warnings 
baseenv()$last.warning
assign("last.warning", NULL, envir = baseenv())
baseenv()$last.warning
```
