...
length(unique(clean_df$lemma))
# -> 14471: 220 unique words dropped. It could be that some of the words were in there with a . and without, so this will reduce the number of unique words if we remove dots. 
...
# THIS NOT NEEEEDED!! NEEDS TO BE CHECKED. SOME MIGHT BE NEEDED AS FOR SOME REASON NUMBERS CHANGES A BIT WHEN SEPARATKY APPLIED ON TOKEN COLUMN BUT SHOULD NOT BE BECAUSE if removing lemma' with numbers are deleted, these tokens are automaticly also delted. It could be that there are token stopwords or token words with numbers that for some reason are different than the lemma and that therefore the numbers do not correspond. 

# Do need to apply the removal of points on token column, have done added this in the chunk above. 

# NEED TO ADD COMMENTS 
# TOKEN
Apply same code above on token instead of lemma
```{r}
...
# we see that the word "" is assigned NOUN  

# -> because there are also other stopwords in there that might have been assigned noun or adj in the upos column and are therefore still included I will have to apply an extra filter that removes the stopwords. This makes sense because some stopwords can indeed be a noun or adj. 
...
# check number of unique words after removing stop words
length(unique(clean_df$token))
# -> 15855: no words dropped because arleady cleaned out with lemma filter applied. 
...
# -> already cleaned out with lemma filter!! 
...
# check number of unique words after removing dots. Should be the same as before. 
length(unique(clean_df$token))
# -> 15603: 220 unique words dropped. It could be that some of the words were in there with a . and without, so this will reduce the number of unique words if we remove dots. 
...
Not that the needed filters have been applied create a file with terms to include. Note that I make one with the tokens (so before stemming/lemmatization) and with the lemma column (so before stemming/lemmatization). 

*NOTE*: I have not done anything yet with - signs in the words.
