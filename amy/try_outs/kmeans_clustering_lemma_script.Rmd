...
lemma_embedding <- subset(vectors_glove, word %in% df_incllemma$filter_lemma)

# check number of unique words
length(unique((lemma_embedding$word)))
# 11529 vs. 12743 lost when token filter is applied.  

# non-GloVe: check which words are in the lemma filter but are not in the feature matrix and are therefore lost (unwanted).
lost_lemma <- subset(df_incllemma, !(filter_lemma %in% lemma_embedding$word))
# 2182 words lost vs. 2117 words lost when token filter applied. 

length(unique((lemma_embedding$word))) + length(unique((lost_lemma$filter_lemma)))
# -> 13711. Which is equal to the line of code below. 
length(unique((df_incllemma$filter_lemma)))
```
...
saveRDS(glove_embedding, "/Users/amyvanderham/Documents/Research_Assistant_Rgit/veni_sysrev/amy/try_outs/glove_embedding_lemma.RData")
```
...
# Try to load pretrained word2vec embedding 
```{r}
library(devtools)
...
# Clustering analysis
Run kmeans with a value for k of 75, 100, 125, 150 and compare the results. 
```{r}
...
# results 
k_means_fit
# 26.9% of the total variance in the data set that is explained by the clustering
```
