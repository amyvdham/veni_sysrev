...
I also save a date frame with only the filter column and keeping only the unique terms. This filter can than be easily applied on the datasets with the pre-trained GloVe and Word2Vec word vectors.  
...
```{r}
# check if file already exists, if not, code will be run to create the file 
filenam7 <- "glove_embedding_final.RData"
...
# create df in which only the words that we want to be included are kept (i.e., extract word vectors for terms in filter data frame from the glove data set)
...
saveRDS(glove_embedding, "/Users/amyvanderham/Documents/Research_Assistant_Rgit/veni_sysrev/amy/glove_embedding_final.RData")
  ...
  glove_embedding <- readRDS(filenam7)
}
```
...
```{r}
# check if file already exists, if not, code will be run to create the file 
filenam8 <- "glove_embedding_bigrams.RData"
if(!file.exists(filenam8)){
  
# load data frame with column with selection of words to include in analysis (bigrams included)
df_bigrams_glove <- readRDS("/Users/amyvanderham/Documents/Research_Assistant_Rgit/veni_sysrev/amy/bigrams_filter_glove.RData")

# create df in which only the words that we want to be included are kept (i.e., extract word vectors for terms in bigrams filter data frame from the glove data set)
...
saveRDS(glove_bigrams_embedding, "/Users/amyvanderham/Documents/Research_Assistant_Rgit/veni_sysrev/amy/glove_embedding_bigrams.RData")
  ...
  glove_bigrams_embedding <- readRDS(filenam8)
}
```
...
```{r}
# check if file already exists, if not, code will be run to create the file 
filenam9 <- "w2v_embedding_final.RData"
...
saveRDS(w2v_embedding, "/Users/amyvanderham/Documents/Research_Assistant_Rgit/veni_sysrev/amy/w2v_embedding_final.RData")
  ...
  w2v_embedding <- readRDS(filenam9)
}
...
```{r}
# check if file already exists, if not, code will be run to create the file 
filenam10 <- "w2v_bigrams_embedding.RData"
...
saveRDS(w2v_bigrams_embedding, "/Users/amyvanderham/Documents/Research_Assistant_Rgit/veni_sysrev/amy/w2v_bigrams_embedding.RData")
  ...
  w2v_bigrams_embedding <- readRDS(filenam10)
}
```
...
# apply filter on the matrix with word vectors exctracted through ASReview so that only the words that we want to be included (i.e., are part of the filter data frame) are kept. 
...
fviz_nbclust(glove_embedding, kmeans, iter.max = 30,  method = "silhouette", k.max = 150) +
  labs(subtitle = "Silhouette method")
# is a lot quick and show the message one time
...
## DBSCAN 
Applying DBSCAN for clustering the terms has also been explored. Based on the steps in the following tutorial: http://www.sthda.com/english/wiki/wiki.php?id_contents=7940
```{r}
