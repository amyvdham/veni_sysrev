...
# find the cluster to which each word belongs
k_means_fit3_cluster43$cluster

# Create data frame in which the merge cluster assignment back to rows/word. 
token_within_clusters <- as.data.frame(cbind(row.names(embedding_cluster43), k_means_fit3_cluster43$cluster))
# add column names
...
k_means_fit9_cluster43 <- kmeans(embedding_cluster43, 9, iter.max = 30, nstart = 25)

sil <- silhouette(k_means_fit9_cluster43$cluster, dist(embedding_cluster43))
fviz_silhouette(sil)
...
resulting_clusters <- as.data.frame(cbind(row.names(embedding_cluster43), k_means_fit9_cluster43$cluster))
# add column names
...
# -> takes a really long time and give converge warning. 
nbclust_out
```
...
# Examine clusters that have significantly fewer observations than other clusters. Clusters that have very few observations may contain outliers or unusual observations with unique characteristics. No clusters with extreme low number of observations. Minimum is 35 and maximum is 270
...
cluster119_78 <- subset(words_with_cluster119, 
                   subset=kmeans119 == 78)
# -> these words seem to be related to statistics, words such as chisquare, varimax, multicollinearity, confounder. But also includes the words zygosity and urbanicity. 
...
cluster119_83 <- subset(words_with_cluster119, 
                    subset=kmeans119 == 83)
# -> this cluster only contains depression and not other forms of this word
...
This code below cannot be run because the memory and time to run this is excessive. 
```{r}
library(clValid)
