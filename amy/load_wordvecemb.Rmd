...
# -> no missing values. 
```
To be able to run cluster analysis etc. on this data I want the words to be the rownames and the columns to be the dimensions. I will this dataframe as a .Rdata object so that I can easily load it into the markdown files in which I run the cluster analysis. The data object can be easily transformed to a matrix by the t() function. 
```{r}
...
saveRDS(df, "amy/featurematrix_asreview.RData")
```
