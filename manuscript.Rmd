...
p <- quote({
  ...
  plot(g1, edge.curved = 0, layout=l1,
       ...
       vertex.frame.width = 2,
         edge.color = "gray70" # edge.color = rgb(0,0,0,.2)
  )
  text(-1.1, y = -1, labels = "a")
  legend(x=-1.2, y=1.1, names(cat_cols), pch=legend_shapes, col=cat_cols, pt.bg=cat_cols, pt.cex=1.2, cex=.7, bty="n", ncol=1)
...
```{r eval = TRUE, message = FALSE, echo = FALSE, warning = FALSE, results="hide"}
...
eval(p)
dev.off()
save_plot("varimps.eps", plot = eval(p), device = cairo_ps, base_height = ((150/200)*7.9), base_width = 7.9)
...
if(run_everything){
  ...
  scale_colour_manual(values = c(Outcome = "gray55", Indicator = "tomato", Cause = "darkgoldenrod3", Protective = "olivedrab4"), guide = NULL)+#
  scale_shape_manual(values = scale_shapes, guide = NULL)+#
  scale_fill_manual(values = c(Outcome = "gray55", Indicator = "tomato", Cause = "darkgoldenrod3", Protective = "olivedrab4"), guide = NULL) +
  scale_x_log10() +
  #scale_y_discrete(labels = italic_labels) +
  scale_linetype_manual(values = c("TRUE" = 3, "FALSE" = 1), guide = NULL) +
...
if(run_everything){
  ...
  p <- quote({
    ...
    text(-1.1, y = -1, labels = "a")
    legend(x=-1.1, y=1.1, names(cat_cols), pch=legend_shapes, col="#777777", pt.bg=cat_cols, pt.cex=1.2, cex=.7, bty="n", ncol=1)
    plot(g, edge.curved = 0, layout=l2,
         vertex.label.family = "Arial",
         vertex.label.cex = 0.5,
         #vertex.shape = "circle2",
...
p <- quote({
  ...
  text(-1.1, y = -1, labels = "a")
  legend(x=-1.1, y=1.1, names(cat_cols), pch=legend_shapes, col=cat_cols, pt.bg=cat_cols, pt.cex=1.2, cex=.7, bty="n", ncol=1)
...
p <- quote({
  ...
  text(-1.1, y = -1, labels = "a")
  legend(x=-1.2, y=1.1, names(cat_cols), pch=legend_shapes, col=cat_cols, pt.bg=cat_cols, pt.cex=1.2, cex=.6, bty="n", ncol=1)
